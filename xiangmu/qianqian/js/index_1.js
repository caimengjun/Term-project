...
//	});
	...
	$('#logout').click(function () {
		$.removeCookie('user');
		window.location.href = 'index.html';//点击了退出 则把cookie中的user清除掉 然后跳转到首页
	});
	...
	$('#reg').dialog({  //注册对话框弹出
		...
		submitHandler : function (form) {  //alert（form） 当验证成功后执行，而且阻止了默认提交 一般用于ajax提交使用
			$(form).ajaxSubmit({
				url : 'php/add.php',
				type : 'POST',
				beforeSubmit : function (formData, jqForm, options) {  //options得到url的值 jqform得到整个表单的对象文本 formdata得到字段里的值
					$('#loading').dialog('open');
					$('#reg').dialog('widget').find('button').eq(1).button('disable'); //注册以后提交按钮变成灰色
				},
				success : function (responseText, statusText) {
					if (responseText) {                       //成功后要回调
						$('#reg').dialog('widget').find('button').eq(1).button('enable');//如果成功了再启用
						$('#loading').css('background', 'url(img/success.gif) no-repeat 20px center').html('数据新增成功...');
						$.cookie('user', $('#user').val());
						setTimeout(function () {
							$('#loading').dialog('close');//一秒钟以后消失
							$('#reg').dialog('close');
							$('#reg').resetForm();
							$('#reg span.star').html('*').removeClass('succ');//去掉对号
		...
		highlight : function (element, errorClass) {
			...
			$(element).parent().find('span').html('*').removeClass('succ');//element获取的是当前的input向上查找再向下找到span 对号
		},
